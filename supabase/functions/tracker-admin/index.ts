...
const temporaryPassword = () => `${crypto.randomUUID().replaceAll("-", "").slice(0, 18)}Aa!`;
...
Deno.serve(async (request) => {
  ...
  if (action === "create_contributor") {
    ...
    const { data: userData, error: userError } = await authAdmin.auth.admin.createUser({ email, password, email_confirm: true });
    ...
    if (memberError) {
      await authAdmin.auth.admin.deleteUser(userData.user.id);
